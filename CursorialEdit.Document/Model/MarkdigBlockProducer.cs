...
public sealed class MarkdigBlockProducer : IDisposable
{
    ...
    private void OnSpliced(SpliceResult result)
    {
        ...
        // Pass 1 — exact content match (same kind, line count, and content hash): a block whose whole
        // text survived the edit unchanged, only shifted, re-adopts here before the anchor pass can be
        // fooled by a stray unmodified line (a trailing blank) drifting into a neighbour. The hash is
        // only ever this equality witness, never the primary key — the block being typed in fails it
        // every keystroke (its text changed) and is re-adopted by the anchor/in-order passes instead.
        ...
        // old block that owned it, disambiguating merges/splits and the edited multi-line block.
        for (var j = 0; j < newWindow; j++)
        {
