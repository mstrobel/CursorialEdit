...
- **[DECISION] Column widths are computed from content** — each column is as wide as its widest cell (header or body), measured in **cells** (`ch`), clamped to a configurable min (**[DECISION]** 3 cells) and max (**[DECISION]** default 40 cells; content beyond wraps or truncates per §5.6).
  - **[DECISION — revised 2026-07-05] Widths are viewport-aware (browser-like auto-layout), not a hard `[3,40]` content clamp.** Natural column widths are still measured from content, but the *rendered* widths are resolved against the **available viewport width** (known at measure time, not parse): if the natural table (Σ column widths + border glyphs) **fits** the viewport, columns grow to their content and **do not wrap** — a column may exceed the old 40-cell cap when the viewport has room (this removes the "most of the viewport is empty yet the column still wraps" defect). If the natural table **overflows** the viewport, the widest column(s) shrink (never below the 3-cell min) and their content **word-wraps** (§5.6) to make the table fit. The old **max width (40)** is retained only as a viewport-unaware **sanity/fallback** cap; the min (3) stays the hard floor. A viewport **resize** re-resolves the widths and re-lays-out the table (§5.5 reflow now has two triggers: content edits *and* resize). Cell wrapping is at **word boundaries** (char-level only for a single word longer than the column).
...
- **[DECISION]** Selecting across cells selects whole cells (a rectangular cell selection *within the table only*) — this is the one place a block-like selection exists, and it is scoped to tables, not the general editor. Copying a cell selection yields the corresponding markdown sub-table.
  - **[IMPL — M3.WP8]** The underlying selection stays an ordinary document **source range** (the model of truth); it is *interpreted* as a whole-cell **rectangle** — rows `[min..max]` × columns `[min..max]` of the cells owning the anchor and active-caret offsets (via `TableModel.CellOfOffset`) — **only when both ends fall in different cells of the same table**. A selection within a single cell stays the ordinary in-cell text selection (WP5); a selection whose ends are in **different blocks** (it left the table into a paragraph) is an ordinary document selection — the table portion then highlights per-cell over the covered span, *not* as a rect (**the transition rule**: a cell-rect requires both ends in the same table). Every cell of the rectangle highlights as a **full cell** (the whole column width incl. padding) under both wire presets (SelectionBrush fill / NoColor Inverse); the interior dividers are **left as normal border glyphs** (the rect reads as highlighted cell-boxes with gridlines between — the spreadsheet convention). **Copy** of a cell-rect writes the selected cells as a valid GFM **sub-table**: the top selected row as the header, a synthesized delimiter carrying the *selected* columns' alignment, then the remaining selected rows — valid even when the rect excludes the model's header row. **Delete / Clear / Cut** over a cell-rect empties every selected cell (structure kept — pipes, the delimiter line, and non-selected cells copied verbatim) as **one** sealed structural undo group, with the caret landing in the rect's top-left cell.
...
- **[DECISION]** When a column would exceed its max width (§5.1), its cell content **wraps within the cell** (the row grows taller, cell text flows to multiple lines inside the box). Alternative: **truncate with `…`** and reveal full text on cell focus — **[DECISION]** wrapping is the v1 default; truncation is a configurable mode.
  - **[IMPL — M3.WP6]** The mode is a settable `OverflowMode` (default `Wrap`; the user-facing command lands with M5). Under `Truncate`, each over-wide cell renders on **one** visual row clipped to its column with a trailing `…` (U+2026), so a logical row is exactly one visual row. **Reveal-on-focus:** the focused cell re-draws its full content un-truncated, overflowing rightward into its neighbours on that content row (spreadsheet-style) and clipped before the grid's right border; siblings stay truncated. The caret map maps cells at their natural full geometry, so the caret lands consistently on the drawn prefix and the reveal alike. *(Deferred: a caret deep in a very long non-terminal cell can reach the clip edge — a per-cell caret-following slide within the box is a later refinement.)*
- **[EDGE]** A table wider than the whole viewport even after wrapping: the table region gets its own horizontal scroll; the rest of the document does not scroll horizontally.
  - **[IMPL — M3.WP6]** This is a **presenter-internal column-window**, not a nested `ScrollViewer` (FB-6 sidestep): engaged automatically in *either* cell mode when the grid can't fit even at `MinWidth` columns. The whole grid draws through a horizontal cell offset (column-granular) that **follows the caret** — editing/Tab into an off-window column scrolls it on-screen. The document's own horizontal extent stays pinned to the viewport (`Extent.Columns == viewport`, `HorizontalOffset == 0`), so the document itself never scrolls sideways.
